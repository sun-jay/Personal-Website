  ...
  return (
    <div className="">
      ...
      <div className="w-full h-auto flex items-center justify-center text-white bg_thatblack">
          ...
          <div onClick ={()=>setTab('all')} className={tab=="all"?"text-2xl w-6/12 text-center p-4 border-b-8 testglow border-b-red-300 ease-in duration-100":
          "text-2xl w-6/12 text-center p-4 ease-in duration-100 "}>
            All
          </div>
          <div  onClick ={()=>setTab('music')} className={tab=="music"?"text-2xl w-6/12 text-center p-4 border-b-8 testglow border-b-red-300  ease-in duration-100":
          "text-2xl w-6/12 text-center p-4 ease-in duration-100 "}>
            Music
          </div>
          <div onClick ={()=>setTab('tech')} className={tab=="tech"?"text-2xl w-6/12 text-center p-4 border-b-8 testglow border-b-red-300  ease-in duration-100":
          "text-2xl w-6/12 text-center p-4 ease-in duration-100 "}>
            Tech
          </div>

      </div>
      {tab == "all"?( arrjsx.map((p)=>p) ):(<div></div>)}
